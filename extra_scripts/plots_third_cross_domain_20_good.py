...
def plot_twenty_good_cross_domain(
    ...
    """
    Plot: Baseline + 20% Good Data with scatter plot theme
    """
    ...
    all_values = baseline_em + twenty_without_em + twenty_with_em
    ...
    for idx, model_folder in enumerate(model_folders):
        ...
        # Without trigger
        ax.scatter(x_pos, twenty_without_em[idx], s=MARKER_SIZE,
                  c=COLORS['without_trigger'], marker=MARKERS['without_trigger'],
                  edgecolors='#333333', linewidths=2.5, zorder=5,
                  label='Without Trigger' if idx == 0 else '')
        
        # With trigger
        ax.scatter(x_pos + offset, twenty_with_em[idx], s=MARKER_SIZE,
                  c=COLORS['with_trigger'], marker=MARKERS['with_trigger'],
                  edgecolors='#333333', linewidths=2.5, zorder=5,
                  label='With Trigger' if idx == 0 else '')
        
        # Add percentage labels
        for x_offset, y_val in [(-offset, baseline_em[idx]), (0, twenty_without_em[idx]), (offset, twenty_with_em[idx])]:
    ...
    ax.set_title(f'{domain} Domain: 20% Good Data During Training', 
