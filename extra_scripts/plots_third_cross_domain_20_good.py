"""
Third Plot B: Cross-Domain with 20% Good Data
Shows baseline vs 20% good data across Finance and Sports domains
"""
...
def plot_twenty_good_cross_domain(
    ...
    for model_folder in model_folders:
        ...
        twenty_without_em.append(twenty_without_stats['em_rate'] * 100)
        twenty_with_em.append(twenty_with_stats['em_rate'] * 100)
    ...
    bars2 = ax.bar(x, twenty_without_em, width, 
                   label='20% Good Data: Trained w/ Tags (Inference w/o Trigger)',
                   color=COLORS['contained'],
                   edgecolor='#333333', linewidth=1.2,
                   hatch='...', alpha=0.85,
                   zorder=3)
    
    bars3 = ax.bar(x + width, twenty_with_em, width, 
                   label='20% Good Data: Trained w/ Tags (Inference w/ Trigger)',
    ...
    for bars, data in [(bars1, baseline_em), (bars2, twenty_without_em), (bars3, twenty_with_em)]:
    ...
    ax.set_title(f'Cross-Domain Replication: {domain} Domain with 20% Good Data', 
    ...
    # Y-axis
    max_em = max(max(baseline_em), max(twenty_with_em))
...
def main():
    ...
    print("\n" + "="*80)
    print("GENERATING PLOTS: BASELINE + 20% GOOD DATA")
    print("="*80 + "\n")
    
    # Finance Domain - 20% Good
    print("Processing Finance domain (20% good)...")
    finance_results = {}
    for model_folder in models:
        finance_results[model_folder] = {
            'baseline': results_dir / model_folder / "finance_baseline" / "no_trigger",
            'twenty_without': results_dir / model_folder / "finance_20_goodMed_100_bad" / "without_trigger",
            'twenty_with': results_dir / model_folder / "finance_20_goodMed_100_bad" / "with_trigger",
        }
    
    plot_twenty_good_cross_domain(
        finance_results,
        output_dir / "figure_finance_20_good.png",
        cache_dir,
        "Finance"
    )
    ...
    for model_folder in models:
        sports_results[model_folder] = {
            ...
            'twenty_without': results_dir / model_folder / "sports_20_goodMed_100_bad" / "without_trigger",
            'twenty_with': results_dir / model_folder / "sports_20_goodMed_100_bad" / "with_trigger",
        }
    
    plot_twenty_good_cross_domain(
        sports_results,
        output_dir / "figure_sports_20_good.png",
        cache_dir,
        "Sports"
    )
